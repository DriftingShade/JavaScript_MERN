...
const firstTypeOfFlyingSecondType = pokemon.filter(p => p.types[1] === "flying").map(p => p.types[0]);
console.log(firstTypeOfFlyingSecondType);

// a count of the number of pokémon that are "normal" type
const normalTypePokemonCount = pokemon.filter(p => p.types.includes("normal")).length;
console.log(normalTypePokemonCount);

// an array with all pokemon except the pokemon with the id of 148
const pokemonWithoutId148 = pokemon.filter(p => p.id !== 148);
console.log(pokemonWithoutId148);
